    ...
    <section className="rounded-lg border border-border bg-surface-2 p-4">
      ...
      <div className="flex flex-wrap gap-2">
        {entries.map(({ section, result }) => (
          ...
          >
            <span className="text-xs font-medium font-mono capitalize">{result.tool}</span>
            <span className="h-1 w-1 rounded-full bg-current opacity-40" />
            <span className="text-[11px] uppercase tracking-wider">{result.status}</span>
            <span className="text-[10px] text-faint/70 capitalize">{section}</span>
          </div>
        ))}
      </div>
    </section>
