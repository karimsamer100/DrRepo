...
export function RecentAudits({ items, onSelect, onClear }: RecentAuditsProps) {
  ...
  const renderItem = (item: RecentAudit, index: number) => {
    ...
    return (
      <li key={`${item.sourceType}-${item.sourceLabel}-${item.profile}-${index}`}>
        <button
          ...
          className="group w-full rounded-xl border border-border bg-base p-2.5 text-left transition-colors duration-150 hover:border-brand/30 hover:bg-surface-2"
        >
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex min-w-0 items-center gap-2">
                <span className="rounded-full border border-border bg-surface px-1.5 py-0.5 text-[12px] uppercase text-faint">
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                <span className="rounded-full border border-border bg-surface px-1.5 py-0.5 text-[12px] text-faint">
                  {item.profile}
                </span>
                {item.analysisMode && (
                  <span className="rounded-full border border-border bg-surface px-1.5 py-0.5 text-[12px] text-faint">
                ...
                {!!item.blockerCount && (
                  <span className="rounded-full border border-error/30 bg-error/10 px-1.5 py-0.5 text-[12px] text-error">
            ...
            <div className="shrink-0 text-right">
              <div className={`font-mono text-base font-semibold leading-none ${scoreColor(item.overallScore)}`}>
                {scoreText}
              </div>
              <div className="mt-0.5 text-[12px] capitalize text-faint">{verdict}</div>
            </div>
          </div>
        </button>
      </li>
    ...
    <div className="rounded-xl border border-border bg-base/60 p-3">
      ...
      {extraItems.length > 0 && (
        <details className="mt-2">
          <summary className="cursor-pointer text-[13px] text-muted transition-colors hover:text-primary">
            Show more rerun shortcuts ({extraItems.length})
          </summary>
          <ul className="mt-2 space-y-2">
            {extraItems.map((item, index) => renderItem(item, index + 2))}
          </ul>
        </details>
      )}

      <p className="mt-3 text-[12.5px] leading-5 text-faint/80">
